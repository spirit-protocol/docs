...
<Steps>
  <Step title="Issue an API token">
    On the agent's **Reach** page in Studio, issue a `sat_` credential. It is shown
    once — copy it.
  </Step>
...
instantly. The authoritative state remains in Studio and its portable identity
data can be carried in an [exported suitcase](/studio-api/portability).
