...
All programmatic access uses bearer credentials, hashed at rest, shown once when
issued, and revocable at any time.
...
- **Agent token (`sat_`)** — issued by the owner on the agent's **Reach**
  page in the studio.
...
The key is **created at the moment of delivery** — its plaintext is never
