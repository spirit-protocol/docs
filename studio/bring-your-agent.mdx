...
If your agent is code you run but it has no HTTP surface, put this minimal
authenticated adapter in front of it (Node, no dependencies):
...
createServer(async (req, res) => {
  ...
  if (req.method !== "POST" || req.url !== "/api/chat")
    return res.writeHead(404).end(JSON.stringify({ error: "not_found" }));
  ...
  try {
    ...
    for await (const chunk of req) {
      ...
      if (body.length > 32_000)
        return res.writeHead(413).end(JSON.stringify({ error: "too_large" }));
    }
    const { message, sessionId } = JSON.parse(body || "{}");
    if (typeof message !== "string" || !message.trim() || message.length > 8_000)
      return res.writeHead(400).end(JSON.stringify({ error: "invalid_message" }));
